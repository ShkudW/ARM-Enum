...
function Export-ResultsFileToHtml {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$DataPath, 
        [Parameter(Mandatory)][string]$Path, 
        [string]$Title = "Azure Key Vault - Extraction Report"
    )
    ...
    if ($raw.TrimStart().StartsWith("[")) {
        ...
        $lines = Get-Content -Path $DataPath -Encoding UTF8 | Where-Object { $_.Trim() }
        $json = "[`n" + ($lines -join ",`n") + "`n]"
    }

    
    $jsonEsc = $json -replace '</script>', '<\/script>'
...
<style>
  ...
  .card{background:#111827;border:1px solid #2a3340;border-radius:14px;padding:16px;box-shadow:0 1px 12px rgba(0,0,0,.25)}
  .wrap{white-space:pre-wrap;word-break:break-word}
  .summary{opacity:.85}
  .badge{display:inline-block;background:#1f2937;padding:2px 8px;border-radius:999px;border:1px solid #374151;font-size:12px;margin-right:6px}
  details summary{cursor:pointer}
  table.dataTable thead th{background:#0f172a;color:#e6edf3;border-bottom:1px solid #334155}
...
<h1>$Title</h1>
<p class="summary">
  <span class="badge" id="countSecrets">Secrets: 0</span>
  <span class="badge" id="countKeys">Keys: 0</span>
  <span class="badge" id="countCerts">Certificates: 0</span>
</p>

<section class="card">
  <h2>Secrets</h2>
  <table id="secrets" class="display" style="width:100%">
    <thead>
      <tr>
        <th>Subscription</th><th>Resource Group</th><th>Key Vault</th><th>Name</th><th>Value</th>
      </tr>
    </thead>
    <tbody></tbody>
  </table>
...
<section class="card">
  <h2>Keys</h2>
  <table id="keys" class="display" style="width:100%">
    <thead>
      <tr>
        <th>Subscription</th><th>Resource Group</th><th>Key Vault</th><th>Name</th><th>KeyId</th><th>KeyOps</th>
      </tr>
    </thead>
    <tbody></tbody>
  </table>
...
<section class="card">
  ...
  <table id="certs" class="display" style="width:100%">
    <thead>
      <tr>
        <th>Subscription</th><th>Resource Group</th><th>Key Vault</th><th>Name</th><th>Type</th><th>Data</th>
      </tr>
    </thead>
    <tbody></tbody>
  </table>
...
<script id="kvdata" type="application/json">
$jsonEsc
</script>
...
<script>
(function(){
  const raw = document.getElementById('kvdata').textContent;
  /** @type {Array<Object>} */
  const data = JSON.parse(raw);

  const isSecret = x => x.ResourceType === 'KeyVault-Secret' || x.ResourceType === 'KeyVault-Secret-Pair';
  const isKey    = x => x.ResourceType === 'KeyVault-Key';
  const isCert   = x => x.ResourceType === 'KeyVault-Certificate' || x.ResourceType === 'KeyVault-Certificate-PFX';

  const secrets = data.filter(isSecret);
  ...
  const pickName = r => r.SecretName ?? r.SecretBaseName ?? r.Name ?? '';
  const pickSecretVal = r => {
    if ('SecretValue' in r && r.SecretValue) return r.SecretValue;
    const parts=[];
    if ('Username' in r && r.Username) parts.push(`username=${r.Username}`);
    if ('Password' in r && r.Password) parts.push(`password=${r.Password}`);
    if ('Value' in r && r.Value)       parts.push(r.Value);
    return parts.join('; ');
  };
  const shortify = (s, n=160) => s && s.length>n ? s.substring(0,n)+'...' : s;

  // SECRETS
  new DataTable('#secrets', {
    data: secrets,
    columns: [
      {data:'SubscriptionName', title:'Subscription', defaultContent:''},
      {data:'ResourceGroup',    title:'Resource Group', defaultContent:''},
      {data:'ResourceName',     title:'Key Vault', defaultContent:''},
      {data:row=>pickName(row), title:'Name'},
      {data:row=>{
        const full = pickSecretVal(row) ?? '';
        const sh = shortify(full);
        return `<details><summary>${esc(sh)}</summary><pre class="wrap">${esc(full)}</pre></details>`;
      }, title:'Value'}
    ],
    pageLength: 25, stateSave: true, deferRender: true, order: [[0,'asc'],[2,'asc'],[3,'asc']]
  });
  ...
  new DataTable('#keys', {
    ...
    columns: [
      ...
      {data:'ResourceGroup',    title:'Resource Group', defaultContent:''},
      {data:'ResourceName',     title:'Key Vault', defaultContent:''},
      {data:'KeyName',          title:'Name', defaultContent:''},
      {data:'KeyId',            title:'KeyId', defaultContent:'', render:(d)=>`<span class="wrap">${esc(d??'')}</span>`},
      {data:row=>{
        const ops = row.KeyOps;
        return Array.isArray(ops) ? esc(ops.join(',')) : esc(ops??'');
      }, title:'KeyOps'}
    ],
    pageLength: 25, stateSave: true, deferRender: true, order: [[0,'asc'],[2,'asc'],[3,'asc']]
  });
  ...
  new DataTable('#certs', {
    ...
    columns: [
      ...
      {data:'ResourceGroup',    title:'Resource Group', defaultContent:''},
      {data:'ResourceName',     title:'Key Vault', defaultContent:''},
      {data:'CertificateName',  title:'Name', defaultContent:''},
      {data:row=>{
        if (row.ResourceType === 'KeyVault-Certificate' && row.CerBase64Der) return 'Public (CER)';
        if (row.ResourceType === 'KeyVault-Certificate-PFX' && row.PfxBase64) return 'PFX (Secret)';
        return '';
      }, title:'Type'},
      {data:row=>{
        const v = row.CerBase64Der ?? row.PfxBase64 ?? '';
        const sh = shortify(v);
        return v ? `<details><summary>${esc(sh)}</summary><pre class="wrap">${esc(v)}</pre></details>` : '';
      }, title:'Data'}
    ],
    pageLength: 25, stateSave: true, deferRender: true, order: [[0,'asc'],[2,'asc'],[3,'asc']]
  });
...
function main{
    ...
    Export-ResultsFileToHtml -DataPath $dataPath -Path .\KeyVault_Report.html -Title "Key Vault Extraction Report"
